...
{
    public class LubracationRollValidator : AbstractValidator<LubracationRoll>
    {
        public LubracationRollValidator()
        {
            RuleFor(x => x.Id).NotEmpty();

            RuleFor(x => x.RollNumber).NotEmpty();
            RuleFor(x => x.RollNumber).Length(2, 30);
            ...
            RuleFor(x => x.Roughness).Length(2, 40);

            RuleFor(x => x.Status).Length(2, 30);

            RuleFor(x => x.Location).Length(2, 40);
